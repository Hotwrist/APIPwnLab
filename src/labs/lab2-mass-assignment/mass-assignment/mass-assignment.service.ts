...
export class MassAssignmentService {
    ...
    async updateUser(userId: number, reqBody: UpdateUserDto, headers: any) {
        ...
        if (reqBody.role) {
            ...
            await this.labsService.markTaskSolved(userId, "Mass Assignment", "Modify your own role");
        }

        if (reqBody.role === "admin" && headers['x-admin-override'] == "true") {
            user.role = "admin";
            await this.labsService.markTaskSolved(userId, "Mass Assignment", "Become an admin");
        }
        
        if (reqBody.isVerified) {
            user.isVerified = reqBody.isVerified;
            await this.labsService.markTaskSolved(userId, "Mass Assignment", "Modify protected account fields (Become Verified)");
        }

        if(reqBody.isPremium) { user.isPremium = false; }

        if (reqBody.voucherCode ) {
            const voucher = await this.voucherRepo.findOne({ where: {code: reqBody.voucherCode}});

            if(!voucher) return { message: `Invalid voucher: ${reqBody.voucherCode}` }

            user.voucherCode = reqBody.voucherCode;
            
            await this.labsService.markTaskSolved(userId, "Mass Assignment", "Modify protected account fields (Become Premium)");
